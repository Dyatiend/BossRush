...
public class OnMouseLook : MonoBehaviour
{
    ...
    void Update()
    {
        // Generate a plane that intersects the transform's position with an upwards normal.
			Plane playerPlane = new Plane(Vector3.up, centerChar.position);
			...
			if (playerPlane.Raycast(ray, out hitdist))
			{
				...
				targetPoint.y = centerChar.position.y;
				Debug.DrawLine(centerChar.position, targetPoint, Color.black);

				// Determine the target rotation.  This is the rotation if the transform looks at the target point.
				Quaternion targetRotation = Quaternion.LookRotation(targetPoint - centerChar.position);
	...
	void FixedUpdate()
    {
		...
		//Debug.Log(centerChar.localPosition);
